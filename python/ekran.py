...
import json
from serwis import zmiana_stanu,cisnienie,wylacz,temperatura,natezenie,napiecie
...
try:
    def  serwisowy(zamknij):
        ...
        def watek():
            ...
            global outd
            while a==True:
                dana = f"cisnienie {cisnienie()} Pa  temp {temperatura()}*C   napiecie {napiecie()}Kv natezenie{natezenie()}mA"
                outd.config(text=dana)
                time.sleep(1)


        threading.Thread(target=watek).start()
        ...
        wyjscie=tk.Button(serw,
            ...
            fg="black",
                          ...
                          height=1,
            font=("Helvenica",20),
            command=lambda: [wylacz(),main(serw),wylaczwatek()]
            )
